...
const empty = () => ({
  ...
  gstNumber: '', address: '', city: '', state: '', remarks: '', isActive: true,
});
...
export default function PrintersPage() {
  ...
  const fetchPrinters = useCallback(async () => {
    ...
    try {
      ...
      const res: any = await api.get(`/printers?${q.toString()}`);
      if (res && res.success) {
    ...
    catch (e: any) { setPageErr(e?.message || 'Failed to load printers'); }
  ...
  const openEdit = (p: Printer) => {
    setEditingId(p.id);
    setForm({ name: p.name, contactPerson: p.contactPerson || '', mobile: p.mobile || '',
      email: p.email || '', gstNumber: p.gstNumber || '', address: p.address || '',
      city: p.city || '', state: p.state || '', remarks: p.remarks || '', isActive: p.isActive });
    setFormErr(''); setShowModal(true);
  };

  const save = async () => {
    if (!form.name.trim()) { setFormErr('Printer Name required'); return; }
    setSaving(true); setFormErr('');
    const payload = { name: form.name.trim(), contactPerson: form.contactPerson || undefined,
      mobile: form.mobile || undefined, email: form.email || undefined,
      gstNumber: form.gstNumber || undefined, address: form.address || undefined,
      city: form.city || undefined, state: form.state || undefined,
      remarks: form.remarks || undefined, isActive: form.isActive };
    try {
      if (editingId) await api.patch(`/printers/${editingId}`, payload);
      else await api.post('/printers', payload);
      setShowModal(false); fetchPrinters();
  ...
  const exportCSV = () => {
    const rows = [['Code','Name','Contact','Mobile','Email','GST','City','State','Status'],
      ...filtered.map(p => [p.printerCode,p.name,p.contactPerson||'',p.mobile||'',
        p.email||'',p.gstNumber||'',p.city||'',p.state||'',p.isActive?'Active':'Inactive'])];
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([rows.map(r=>r.map(c=>`"${c}"`).join(',')).join('\n')],{type:'text/csv'}));
    a.download = 'printers.csv'; a.click();
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-bold text-gray-100 tracking-tight">Printer Management</h2>
          <p className="text-xs text-gray-500 mt-0.5">{meta.totalItems} printers registered</p>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={exportCSV}
            className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/5 text-[11px] text-gray-400 hover:text-white hover:bg-white/10 transition-colors flex items-center gap-1 cursor-pointer">
          ...
          <button onClick={openAdd} id="btn-add-printer"
      ...
      <div className="glass-card rounded-lg px-4 py-2.5 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px] max-w-xs">
          ...
          <input id="printer-search" type="text" placeholder="Search name, contact, city..."
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)}
          className="bg-[#080c14] border border-white/5 rounded-lg px-4 py-2.5 text-sm text-gray-300 focus:outline-none focus:border-indigo-500/50 cursor-pointer">
          <option value="all">All Status</option>
          <option value="active">Active</option>
          <option value="inactive">Inactive</option>
        </select>
        <button className="px-4 py-2.5 rounded-lg bg-white/5 border border-white/5 text-sm text-gray-400 hover:text-white transition-colors cursor-pointer">
          ⚙ Filter
        </button>
          ...
          <table className="w-full text-left border-collapse" style={{ fontSize: '13px' }}>
            <thead>
              <tr className="bg-black/20 border-b border-white/5">
                {['PRINTER NAME','CONTACT PERSON','MOBILE & EMAIL','GST NUMBER','CITY / STATE','STATUS','ACTIONS'].map(h => (
              ...
              {loading ? (
                <tr><td colSpan={7} className="px-3 py-10 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[11px] text-gray-600">Loading printers…</span>
                  </div>
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-3 py-12 text-center">
                  <p className="text-[11px] text-gray-600">No printers found. <button onClick={openAdd} className="text-indigo-400 hover:underline">+ Add new</button></p>
                </td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} className="hover:bg-white/[0.025] transition-colors group">
                  {/* Name */}
                  <td className="px-3 py-2">
                    <div className="font-semibold text-gray-100 leading-tight">{p.name}</div>
                    <div className="text-[11px] text-indigo-400 font-mono mt-0.5">{p.printerCode}</div>
                  </td>
                  {/* Contact */}
                  <td className="px-4 py-3 text-gray-300">
                    {p.contactPerson || <span className="text-gray-700">—</span>}
                  </td>
                  {/* Mobile & Email */}
                  <td className="px-3 py-2">
                    <div className="text-gray-200 font-medium">{p.mobile || '—'}</div>
                    {p.email && <div className="text-[10px] text-sky-400 mt-0.5">{p.email}</div>}
                  </td>
                  {/* GST */}
                  <td className="px-4 py-3 font-mono text-gray-400" style={{ fontSize: '12px' }}>
                    {p.gstNumber || <span className="text-gray-700">—</span>}
                  </td>
                  {/* City / State */}
                  <td className="px-3 py-2">
                    <div className="text-gray-200 font-medium">{p.city || '—'}</div>
                    {p.state && <div className="text-[11px] text-gray-500 mt-0.5">{p.state}</div>}
                  </td>
                  {/* Status */}
                  <td className="px-3 py-2">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${p.isActive ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
        ...
        <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div className="glass-card rounded-2xl p-5 w-full max-w-lg max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl">
            ...
            <div className="space-y-3">
              {/* Printer Name */}
              <div>
                <label className={lbl}>Printer Name <span className="text-red-400">*</span></label>
                <input id="field-name" type="text" placeholder="e.g. Rainbow Printers"
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input id="field-contact" type="text" placeholder="e.g. Naresh Patel"
                    value={form.contactPerson} onChange={e => setForm(f => ({ ...f, contactPerson: e.target.value }))} className={inp}/>
                </div>
                <div>
                  <label className={lbl}>Mobile Number</label>
                  <input id="field-mobile" type="tel" placeholder="e.g. 9825456789"
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input id="field-email" type="email" placeholder="e.g. print@rainbow.com"
                    value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className={inp}/>
                </div>
                <div>
                  <label className={lbl}>GST Number</label>
                  <input id="field-gst" type="text" placeholder="24DDDP1234P1Z4"
              ...
              <div>
                ...
                <input id="field-address" type="text" placeholder="Street, GIDC Sector..."
                  value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} className={inp}/>
              </div>
              ...
              <label className="flex items-center gap-2.5 cursor-pointer p-2.5 rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.05] transition-colors"
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-600">Printer is available for print jobs</div>
                </div>
              </label>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-4 pt-3 border-t border-white/5">
              <button onClick={() => setShowModal(false)} id="btn-cancel-printer"
              ...
              <button onClick={save} disabled={saving} id="btn-save-printer"
                className="px-4 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-900 text-white text-[11px] font-semibold transition-colors cursor-pointer shadow-lg shadow-emerald-500/20 flex items-center gap-1.5">
                {saving
